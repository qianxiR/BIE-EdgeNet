...
num_gpu=$3
work_dirs=$4
model_checkpoint=$(find "$work_dirs" -name 'best_IoU_iter_*.pth' -type f -print -quit)
# model_checkpoint=$(find "$work_dirs" -name 'iter_35000.pth' -type f -print -quit)
...
if [ "$data_name" == "WHUCD" ]; then
    ...
    python tools/general/metric.py --pppred "$work_dirs/test_result" --gggt "$label_dir"
elif [ "$data_name" == "PXCLCD" ]; then
    label_dir="$CDPATH/PX-CLCD/test/label"
    bash tools/dist_test.sh "$config_file" "$model_checkpoint" $num_gpu --out "$work_dirs/test_result" --cfg-options test_dataloader.dataset.data_root="$CDPATH/PX-CLCD"
    python tools/general/metric.py --pppred "$work_dirs/test_result" --gggt "$label_dir"
    ...
    python tools/general/metric.py --pppred "$work_dirs/test_result" --gggt "$label_dir"
elif [ "$data_name" == "LEVIRSLIDE" ]; then
    label_dir="$CDPATH/LEVIR-CD/test/label"
    bash tools/dist_test.sh "$config_file" "$model_checkpoint" $num_gpu --out "$work_dirs/test_result_slide"
    python tools/general/metric.py --pppred "$work_dirs/test_result_slide" --gggt "$label_dir"
elif [ "$data_name" == "BANDON" ]; then
    label_dir="$CDPATH/LEVIR-CD/test/label"
    bash tools/dist_test.sh "$config_file" "$model_checkpoint" $num_gpu --out "$work_dirs/test_result_slide"
    python tools/general/metric.py --pppred "$work_dirs/test_result_slide" --gggt "$label_dir"
elif [ "$data_name" == "S2Looking" ]; then
    label_dir="$CDPATH/S2Looking/test/label"
    bash tools/dist_test.sh "$config_file" "$model_checkpoint" $num_gpu --out "$work_dirs/test_result"
    python tools/general/metric.py --pppred "$work_dirs/test_result" --gggt "$label_dir"
