...
model = dict(
    ...
    auxiliary_head=
        ...
        out_channels=1,
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=True, loss_weight=0.33)),
        dict(
        type='NullDecodeHead',
        in_index=2,
        in_channels=256,
...
val_dataloader = dict(batch_size=12,   #修改batchsize
                        num_workers=12,
                        dataset=dict(data_root=data_root))
test_dataloader = dict(batch_size=12,   #修改batchsize
                        num_workers=12,
                        dataset=dict(data_root=data_root))

# training schedule for 20k
train_cfg = dict(type='IterBasedTrainLoop', max_iters=30000, val_interval=500)  #修改训练次数的
