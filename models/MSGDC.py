...
class Config:
    def __init__(self):
        ...
        self.weight_bits = 32# 权重量化位数 可以为 1，2，＜32 分别对应 BinaryQuantizer、TwnQuantizer、SymQuantizer
        self.input_bits = 32# 输入量化位数 与self.weight_bits要对应
...
class MultiScale_Grouped_Dilated_Convolution(nn.Module):
    def __init__(self, in_chn, config, dilation1=1, dilation2=3, dilation3=5, kernel_size=3, stride=1, padding='same'):
        ...
        self.groups = min(4, in_chn // 16)
        # 初始化不同扩张率的量化卷积层（分组数为4）
        self.cov1 = QuantizeConv2d(in_chn, in_chn, kernel_size, stride, padding, dilation1, self.groups, bias=True, config=config)
        self.cov2 = QuantizeConv2d(in_chn, in_chn, kernel_size, stride, padding, dilation2, self.groups, bias=True, config=config)
        self.cov3 = QuantizeConv2d(in_chn, in_chn, kernel_size, stride, padding, dilation3, self.groups, bias=True, config=config)
    ...
    def forward(self, x):
        ...
        return x.permute(0, 2, 1).view(-1, C, H, W).contiguous() + residual * self.res_scale
