...
class DiffOnlyBlock(nn.Module):
    """
    完全替换原Block的Attention为差分注意力（OCDA/CDA）
    保留原Block的所有接口和剩余逻辑（norm1/norm2、drop_path、mlp）
    """
    def __init__(
        self,
        dim,
        ...
        sr_ratio=1,
        # 差分注意力参数
        diff_attn_type="OCDA",  # OCDA/CDA
        window_size=8,
        overlap_ratio=0.25,
        depth=1,
    ):
        super().__init__()
        # 保留原Block的基础结构（接口完全一致）
        ...
        # 核心替换：原Attention → 差分注意力（OCDA/CDA）
        if diff_attn_type == "OCDA":
            self.attn = OCDA(
                dim=dim,
                num_heads=num_heads,
                depth=depth,
    ...
    def forward(self, x, H, W):
        # 完全复用原Block的forward逻辑，仅替换attn实现
        x = x + self.drop_path(self.attn(self.norm1(x), H, W))  # 差分注意力直接替代原Attention
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))   # 保留原MLP
        return x